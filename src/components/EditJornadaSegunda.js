...
const EditJorPrimera = () => {
    ...
    const update = async (e) => {
        e.preventDefault()
            ...
            const product = doc(db, "jornadasSegunda", id)
            const data = {cancha: cancha, equipo1:equipo1, equipo2:equipo2, fecha: fecha, hora: hora, jornada:jornada}
            await updateDoc(product, data)
            //console.log(result)
            navigate('/segundajornadas')
   
        
    }

    const getProductById = async (id) => {
        const product = await getDoc( doc(db, "jornadasSegunda", id) )
        if(product.exists()) {
