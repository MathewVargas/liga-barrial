...
const Dashboard = () => {
  ...
  const q1 = query(equiposCollection, where("categoria", "==", "primera"), orderBy("puntos", "asc"));
      ...
      <div className="row">
        {equipos.map((equipo) => (
          <div className="col-md-3 mb-4" key={equipo.id}>
            <Link to={`/jugadoresPrim/${encodeURIComponent(equipo.nombre_equip)}`} className="card-link" style={{ textDecoration: 'none' }}>
              <div className="card">
